...
Modified the ANTLR grammar file Directives.g4 to:
Add new lexer tokens: BYTE_SIZE, TIME_DURATION
Add supporting fragments: BYTE_UNIT, TIME_UNIT
Extend parser rules to accept BYTE_SIZE and TIME_DURATION as valid argument values
Regenerated the lexer and parser classes using mvn compile

API Updates (wrangler-api module)
Introduced two new token classes:
ByteSize.java: Parses and stores values like 10KB, 1.5MB, and exposes the value in canonical unit (bytes)
TimeDuration.java: Parses and stores values like 500ms, 2s, and exposes the value in canonical unit (nanoseconds)
Registered new token types BYTE_SIZE and TIME_DURATION in TokenType.java
Updated token and usage definitions to support the new token types

Core Parser Updates (wrangler-core module)
Implemented visitByteSizeArg and visitTimeDurationArg methods in the visitor class to handle new parser rules
Added logic to construct ByteSize and TimeDuration token instances and store them in the TokenGroup
Modified visitor logic to include the new types in recipe parsing

New Directive: aggregate-stats
A new aggregate directive was introduced with the following behavior:
Class Location: C:\Users\LC\Desktop\zeotapsubmission\wrangler\wrangler-core\src\main\java\io\cdap\wrangler\directive\AggregateStats.java
Directive Interface: Implements Directive, with aggregation capability

Arguments:
Source column name for byte size values
ource column name for time duration values
Target column name for total size
Target column name for total or average time
(Optional) Output unit and aggregation type (e.g., total, average, unit=MB, unit=s)

Execution:
Uses ExecutorContext store to maintain running totals
Converts input values to canonical units (bytes, nanoseconds)
At finalization, converts totals to desired units
Emits a single output row containing the aggregate metrics

Testing
Unit Tests
ByteSize Parsing:
Validated canonical values for inputs such as 10KB, 1.5MB, 3GB, 1024B
TimeDuration Parsing:
Validated correct conversion of 500ms, 2s, 1.5min, 100ns to nanoseconds
Token Edge Cases:
Verified case-insensitive parsing and handling of invalid unit formats
Parser Tests
Added tests to verify correct parsing of recipes with BYTE_SIZE and TIME_DURATION token types
Validated recipe parsing and token creation in GrammarBasedParserTest.java and RecipeCompilerTest.java
Aggregate Directive Tests
Added integration tests using TestingRig to validate:
Byte size aggregation and unit conversion (e.g., to MB)
Time duration aggregation and conversion (e.g., to seconds)
Correct output for total and average modes
